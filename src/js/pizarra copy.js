...
function handleMouseDown(e) {
    ...
    if (currentTool === 'pan') {
        ...
        if (clickedObject) {
            // CORRECCIÓN: Seleccionar objeto y preparar para arrastre
            selectObject(clickedObject);
            isDragging = true;
            dragStartX = startX;  // Usar coordenadas del canvas, no de la pantalla
            dragStartY = startY;
...
function renderObject(obj) {
    ...
    switch(obj.type) {
        ...
        case 'line':
            ...
            // CORECCIÓN 3: Preparar para rotación dinámica
            element.style.transformOrigin = 'left center';
            break;
    }

    // Aplicar posición y estilos
...
function getObjectAt(x, y) {
    // Iterar en orden inverso para seleccionar objetos superiores primero
    for (let i = objects.length - 1; i >= 0; i--) {
        const obj = objects[i];
        
        if (obj.type === 'line') {
            // Para líneas rotadas: usar matemática de rotación
            const centerX = obj.x + obj.width / 2;
            const centerY = obj.y + obj.height / 2;
            
            // Rotar el punto de clic hacia el espacio local del objeto
            ...
            // Para círculos: usar distancia radial
            const centerX = obj.x + obj.width / 2;
            const centerY = obj.y + obj.height / 2;
            ...
            // Para rectángulos y texto: detección rectangular simple
...
function updatePropertiesPanel(obj) {
    ...
    // Control especial de rotación para líneas
...
function updateObjectProperty(property, value) {
    ...
    switch (property) {
        ...
        case 'text':
        case 'name':
            processedValue = String(value).substring(0, 100); // Limitar longitud
            break;
        default:
            processedValue = value;
    }
    
    // Aplicar cambio solo si es diferente
    if (selectedObject[property] !== processedValue) {
        selectedObject[property] = processedValue;
        
        // Actualizar DOM según el tipo de propiedad
        if (property === 'text') {
        ...
        // Actualizar panel de propiedades solo si cambió una propiedad visual
